#' Plots the percentage resistance and tested line trend
#'
...
#' @examples
#' Testlinegraph_noCI <- pcentRandT_plot(citable, "Klebsiella", "CIP")
#' @export


pcentRandT_plot <- function(data, filter1, filter2) {
   dt <- filter(data, genus==filter1, antimicrobial==filter2)
  data2 <- xts::xts(x=cbind(dt$Res.PointEst*100, dt$Test.PointEst*100), order.by = dt$year_month)
